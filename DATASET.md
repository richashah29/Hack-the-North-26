...
ask the same question against a broader geographical area. Point `CORPUS_PATH` at it to use it.
...
Hack the North publishes a museum of its finalists, so for HTN we know the actual top ~12 each year. No other event publishes anything like that, so the only thing we can read off their pages is the Devpost prize badge, which includes every sponsor track.

That's why there are two outcome columns. `finalist` keeps the strict meaning for HTN and means "won something" everywhere else, so it is **not** comparable between events.

`won_prize` means "won any prize" for every event including HTN, so that's the one we use if the data is combined together. On that measure HTN sits at 11.2%, which is in the same range as the rest, and the comparison is fair.
